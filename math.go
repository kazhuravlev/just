...
// MaxDefault returns the max element from `in` or the default value for
// a specified type when `in` is empty.
...
// MinDefault returns the min element from `in` or the default value for
// a specified type when `in` is empty.
